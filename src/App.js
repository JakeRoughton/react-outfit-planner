...
function generateOutfit(dressCode) {
    const filteredTops = clothing.filter(item => item.type === 'top' && item.dressCode === dressCode);
    const filteredBottoms = clothing.filter(item => item.type === 'bottom' && item.dressCode === dressCode);
    const filteredShoes = clothing.filter(item => item.type === 'shoes' && item.dressCode === dressCode);
  
    const randomTop = filteredTops[Math.floor(Math.random() * filteredTops.length)];
    const randomBottom = filteredBottoms[Math.floor(Math.random() * filteredBottoms.length)];
    const randomShoes = filteredShoes[Math.floor(Math.random() * filteredShoes.length)];
  ...
  <>
    <div className="App">
      ...
      <button className='Casual btn' onClick={() => generateOutfit('casual')}>Casual</button>
      <br/>
      <button className='Sport btn' onClick={() => generateOutfit('sport')}>Sport</button>
      <br/>
      <button className='Formal btn' onClick={() => generateOutfit('formal')}>Formal</button>
    </div>
    ...
    <div className="outfits">
      {top && (
        <div>
          <h3>Top</h3>
          <img className='img' src={top.imageUrl} alt={top.description} />
        </div>
      )}
      {bottom && (
        <div>
          <h3>Bottom</h3>
          <img className='img' src={bottom.imageUrl} alt={bottom.description} />
        </div>
      )}
      {shoes && (
        <div>
          <h3>Shoes</h3>
          <img className='img' src={shoes.imageUrl} alt={shoes.description} />
        </div>
      )}
    </div>
  </>
